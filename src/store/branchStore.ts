...
export const useBranchStore = create<BranchState>((set, get) => ({
  ...
  fetchBranches: async () => {
    ...
    try {
      ...
      set({ branches: data as Branch[], isLoading: false });
    } catch (err) {
  ...
  addBranch: async (branch) => {
    ...
    try {
      ...
      if (error) throw error;
      set(state => ({ branches: [...state.branches, data as Branch], isLoading: false }));
      return true;
    } catch (err) {
  ...
  updateBranch: async (branchId: string, updates: Partial<Branch>) => {
    ...
    try {
      ...
      if (error) throw error;
      set(state => ({
        branches: state.branches.map(b => b.id === branchId ? { ...b, ...(data as Branch) } : b),
        isLoading: false,
      }));
      return true;
    } catch (err) {
