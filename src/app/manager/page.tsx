    ...
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">สรุปภาพรวมการทำงานประจำวัน {formatThaiDate(new Date().toISOString())}</p>
        </div>
          ...
          <div className="lg:col-span-2 space-y-4">
             <div className="flex items-center justify-between px-1">
                <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                   <Clock className="w-4 h-4 text-primary-600" /> การเข้าใช้งานล่าสุด
                </h2>
                <Link href="/manager/attendance" className="text-xs font-bold text-primary-600 hover:text-primary-700 transition-colors">ดูทั้งหมด</Link>
             </div>
                ...
                <div className="flex-1 overflow-y-auto px-5 space-y-3 pb-5">
                   {pendingSubmissions.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-40 text-center">
                         ...
                         <p className="text-xs text-slate-400 font-medium whitespace-pre-wrap">ยอดเยี่ยม! ตรวจงานครบแล้ว</p>
                      </div>
                   ) : (
                      pendingSubmissions.slice(0,6).map(sub => {
                         ...
                         return (
                            <Link key={sub.id} href={`/manager/review/${sub.id}`}>
                               <div className="p-3 bg-slate-50 rounded-xl border border-transparent hover:border-primary-200 hover:bg-primary-50/30 transition-all flex justify-between items-center group">
                                  <div className="min-w-0">
                                     <p className="text-xs font-bold text-slate-900 truncate">{task?.title || tmpl?.title || 'งาน'}</p>
                                     <p className="text-[10px] text-slate-500 mt-0.5">{emp?.full_name}</p>
                                  </div>
