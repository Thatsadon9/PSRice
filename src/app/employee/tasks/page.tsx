...
export default function MyTasksPage() {
  ...
  const tabs = [
    { id: 'today', label: 'วันนี้', count: allTasks.filter(t => t.due_date === today).length },
    { id: 'week', label: 'สัปดาห์นี้', count: allTasks.filter(t => t.due_date >= today && t.due_date <= weekEndStr).length },
    { id: 'overdue', label: 'เลยกำหนด', count: allTasks.filter(t => t.status === 'overdue').length },
    { id: 'all', label: 'ทั้งหมด', count: allTasks.length },
  ];

  const filteredTasks = (() => {
    switch (activeTab) {
      case 'today': return allTasks.filter(t => t.due_date === today);
      case 'week': return allTasks.filter(t => t.due_date >= today && t.due_date <= weekEndStr);
      case 'overdue': return allTasks.filter(t => t.status === 'overdue');
      default: return allTasks;
    }
  })().sort((a, b) => {
    const statusOrder: Record<string, number> = { overdue: 0, pending: 1, in_progress: 2, submitted: 3, rejected: 4, approved: 5 };
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'overdue': case 'rejected': return <AlertTriangle className="w-4 h-4 text-red-600" />;
      default: return <ClipboardList className="w-4 h-4 text-primary-600" />;
    }
  };
                ...
                <Card interactive className="flex items-start gap-3 mb-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <Badge variant={getStatusVariant(task.status) as 'success' | 'warning' | 'danger' | 'info' | 'default'} size="sm">
                        {TASK_STATUS_LABELS[task.status]}
                      </Badge>
                      {(task.priority || template?.priority) && (
                        <Badge variant={getPriorityVariant(task.priority || template?.priority) as 'danger' | 'warning' | 'info' | 'slate'} size="sm">
                    ...
                    <div className="flex items-center gap-3 mt-1.5 text-xs text-slate-500">
                      ...
                      {task.checklist_state && task.checklist_state.length > 0 && (
                        <span>
                          {task.checklist_state.filter(c => c.completed).length}/{task.checklist_state.length} รายการ
                        </span>
                      )}
                    </div>
                  </div>
