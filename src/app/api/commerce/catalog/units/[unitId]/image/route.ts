...
export async function POST(request: Request, { params }: RouteContext) {
  try {
    ...
    if (!context) return NextResponse.json({ error: 'ไม่มีสิทธิ์แก้ไขรูปหน่วยสินค้า' }, { status: 403 });
    ...
    const objectPath = `catalog/units/${unitId}/${Date.now()}-${randomUUID()}.webp`;
    const { error: uploadError } = await admin.storage
      .from('product-images')
      .upload(objectPath, optimized, { contentType: 'image/webp', cacheControl: '31536000', upsert: false });
    ...
    if (previousPath && previousPath !== objectPath) await admin.storage.from('product-images').remove([previousPath]);
    await admin.from('commerce_audit_logs').insert({
      actor_user_id: context.profile.id,
      action: 'catalog.product_unit.image.updated',
      entity_type: 'product_unit',
      entity_id: unitId,
      payload: { product_id: existing.product_id, unit_name: existing.name, image_url: publicUrl.publicUrl },
    });

    return NextResponse.json({ image_url: publicUrl.publicUrl });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'อัปโหลดรูปหน่วยไม่สำเร็จ' }, { status: 500 });
  }
}
...
export async function DELETE(request: Request, { params }: RouteContext) {
  try {
    const context = await authorized(request);
    if (!context) return NextResponse.json({ error: 'ไม่มีสิทธิ์ลบรูปหน่วยสินค้า' }, { status: 403 });
    const { unitId } = await params;
    const admin = requireSupabaseAdmin();
    const { data: unit, error } = await admin.from('product_units').select('id, product_id, name, image_url').eq('id', unitId).maybeSingle();
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    if (!unit) return NextResponse.json({ error: 'ไม่พบหน่วยสินค้า' }, { status: 404 });

    const { error: updateError } = await admin.from('product_units').update({ image_url: null }).eq('id', unitId);
    if (updateError) return NextResponse.json({ error: updateError.message }, { status: 500 });
    const objectPath = objectPathFromPublicUrl(unit.image_url);
    if (objectPath) await admin.storage.from('product-images').remove([objectPath]);

    await admin.from('commerce_audit_logs').insert({
      actor_user_id: context.profile.id,
      action: 'catalog.product_unit.image.removed',
      entity_type: 'product_unit',
      entity_id: unitId,
      payload: { product_id: unit.product_id, unit_name: unit.name },
    });
    return NextResponse.json({ image_url: null });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'ลบรูปหน่วยไม่สำเร็จ' }, { status: 500 });
  }
}
