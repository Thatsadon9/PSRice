-- Draft goods receipts are intentionally isolated from goods_receipts.  This
-- keeps the existing receipt triggers from touching stock before confirmation.
...
alter table public.commerce_document_counters
  ...
  check (document_type in ('sale', 'return', 'purchase', 'goods_receipt', 'transfer', 'online_order', 'goods_receipt_draft'));

create table if not exists public.goods_receipt_drafts (
  ...
  check (purchase_order_id is not null or supplier_id is not null)
);

create table if not exists public.goods_receipt_draft_items (
  id uuid primary key default gen_random_uuid(),
...
create unique index if not exists goods_receipt_draft_items_po_item_uidx
  on public.goods_receipt_draft_items(draft_id, purchase_order_item_id)
  where purchase_order_item_id is not null;
create index if not exists goods_receipt_drafts_branch_status_idx
  on public.goods_receipt_drafts(branch_id, updated_at desc);
create index if not exists goods_receipt_drafts_branch_received_at_idx
  on public.goods_receipt_drafts(branch_id, received_at desc);
create index if not exists goods_receipt_draft_items_draft_idx
  on public.goods_receipt_draft_items(draft_id);
create index if not exists goods_receipt_draft_items_product_idx
  on public.goods_receipt_draft_items(product_id, product_unit_id);

drop trigger if exists goods_receipt_drafts_set_updated_at on public.goods_receipt_drafts;
create trigger goods_receipt_drafts_set_updated_at
...
revoke all on table public.goods_receipt_drafts, public.goods_receipt_draft_items from anon, authenticated;
grant all on table public.goods_receipt_drafts, public.goods_receipt_draft_items to service_role;
drop policy if exists goods_receipt_drafts_server_only on public.goods_receipt_drafts;
drop policy if exists goods_receipt_draft_items_server_only on public.goods_receipt_draft_items;
create policy goods_receipt_drafts_server_only on public.goods_receipt_drafts
  for all to authenticated using (false) with check (false);
create policy goods_receipt_draft_items_server_only on public.goods_receipt_draft_items
  for all to authenticated using (false) with check (false);
...
create or replace function public.commerce_delete_goods_receipt_draft(
  p_user_id uuid,
  p_branch_id uuid,
  p_draft_id uuid
)
...
begin
  if not public.commerce_has_permission(p_user_id, 'purchasing.receive', p_branch_id)
     and not public.commerce_has_permission(p_user_id, 'purchasing.manage', p_branch_id) then
    raise exception 'ไม่มีสิทธิ์ลบใบนำเข้า';
  end if;
  select * into v_draft from public.goods_receipt_drafts
  where id = p_draft_id and branch_id = p_branch_id for update;
  if not found then raise exception 'ไม่พบใบนำเข้าที่พักไว้'; end if;
  delete from public.goods_receipt_drafts where id = p_draft_id;
  insert into public.commerce_audit_logs(actor_user_id, branch_id, action, entity_type, entity_id, payload)
  values (p_user_id, p_branch_id, 'goods_receipt.draft_deleted', 'goods_receipt_draft', p_draft_id,
    jsonb_build_object('draft_number', v_draft.draft_number));
  return jsonb_build_object('draft_id', p_draft_id, 'draft_number', v_draft.draft_number);
end;
$$;

create or replace function public.commerce_finalize_goods_receipt_draft(
  p_user_id uuid,
  p_branch_id uuid,
  p_draft_id uuid
)
...
begin
  if not public.commerce_has_permission(p_user_id, 'purchasing.receive', p_branch_id)
     and not public.commerce_has_permission(p_user_id, 'purchasing.manage', p_branch_id) then
    raise exception 'ไม่มีสิทธิ์ยืนยันใบนำเข้า';
  end if;
  select * into v_draft from public.goods_receipt_drafts
  where id = p_draft_id and branch_id = p_branch_id for update;
  if not found then raise exception 'ไม่พบใบนำเข้าที่พักไว้'; end if;

  if v_draft.purchase_order_id is not null then
    select coalesce(jsonb_agg(jsonb_build_object(
      'purchase_order_item_id', purchase_order_item_id,
      'quantity', quantity,
      'unit_cost', unit_cost
    )), '[]'::jsonb) into v_items
    from public.goods_receipt_draft_items where draft_id = v_draft.id;
    v_result := public.commerce_receive_purchase_order(
      p_user_id, p_branch_id, v_draft.purchase_order_id, v_items, v_draft.note
    );
  else
    select coalesce(jsonb_agg(jsonb_build_object(
    ...
    from public.goods_receipt_draft_items where draft_id = v_draft.id;
    v_result := public.commerce_receive_goods_direct(
      p_user_id, p_branch_id, v_draft.supplier_id, v_draft.received_at,
      v_draft.payment_method, v_items, v_draft.note
    );
  end if;

  insert into public.commerce_audit_logs(actor_user_id, branch_id, action, entity_type, entity_id, payload)
  values (p_user_id, p_branch_id, 'goods_receipt.draft_finalized', 'goods_receipt_draft', v_draft.id,
    jsonb_build_object('draft_number', v_draft.draft_number, 'result', v_result));
  delete from public.goods_receipt_drafts where id = v_draft.id;
  return v_result || jsonb_build_object('draft_id', v_draft.id, 'draft_number', v_draft.draft_number);
end;
$$;
...
revoke all on function public.commerce_delete_goods_receipt_draft(uuid, uuid, uuid) from public, anon, authenticated;
revoke all on function public.commerce_finalize_goods_receipt_draft(uuid, uuid, uuid) from public, anon, authenticated;
...
grant execute on function public.commerce_delete_goods_receipt_draft(uuid, uuid, uuid) to service_role;
grant execute on function public.commerce_finalize_goods_receipt_draft(uuid, uuid, uuid) to service_role;
